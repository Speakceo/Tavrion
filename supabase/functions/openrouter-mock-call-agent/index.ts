...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { scenarioType, userMessage, conversationHistory } = await req.json();
...
Be pleasant but transactional. Appreciate value-adds beyond just price matching.`
    ...
    const systemPrompt = `You are a customer in a student accommodation sales call. ${scenarios[scenarioType]}

RULES:
- Respond naturally as this character
- Keep responses 2-4 sentences max
- React to what the agent says
- Gradually reveal concerns
- Use filler words like "um", "like"
- Stay in character`;
    ...
    if (!aiResponse || aiResponse.trim() === '') {
      console.warn('Empty response from model, using fallback');
      aiResponse = "Could you tell me more about that?";
    }
